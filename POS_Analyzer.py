...
class POS_Analyzer:
    ...
    def Get_POS_TAG(self , s1):
        W1 = []
        # print "POS Tagging of : ",s1
        wrds = self.Preprocess_Sentence(s1)
    ...
    def make_lists(self , pos_Tagged):
        ADJ = []
        ADP = []
        ADV = []
        CONJ = []
        ...
        return NOUN, VERB, ADJ, ADV, NUM , PRON
    ...
    def word_similarity(self , w1, w2):
        ...
        bw1, bw2 = self.get_best_synset_pair(w1, w2)
        ...
        if sim is None:
            print "Word Similarity : NONE Error :words were :", w1, w2, "and Best Synsets : ", bw1, bw2, "\n\n"
        return sim

    def list_similarity(self , list1, list2):
    ...
    def calculate_similarity(self , sent_1, sent_2 , max_score):  # It takes SENTENCE AS INPUT

        words_1 = self.Get_POS_TAG(sent_1)
        words_2 = self.Get_POS_TAG(sent_2)
        x1 = self.make_lists(words_1)
        x2 = self.make_lists(words_2)
        ...
        for i, j in zip(x1, x2):
            ...
            if len(i) <= 0 or len(j) <= 0:
                empty_count += 1
                continue
            list_sim = self.list_similarity(i, j)
        ...
        similarity = (interp(similarity, [0, 1], [0, max_score]))

        self.sim_score=similarity
        self.pos_tagged1=words_1
        self.pos_tagged2=words_2
